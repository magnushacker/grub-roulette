...
from app.services.recommend import build_candidates, pick_suggestion, search_and_cache_restaurants
...
def search(q: str, lat: float | None = None, lng: float | None = None, db: Session = Depends(get_db), current: User = Depends(get_current_user)):
    """Live text search, used for 'I went somewhere else' lookups. Not cached against radius filters."""
    from app.services.recommend import _upsert_restaurant  # local import to avoid confusing public API surface

    results = google_places.text_search_restaurants(q, lat, lng)
